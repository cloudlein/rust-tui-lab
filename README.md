...
This document describes a sequence of exercises and practice tasks to learn
how to build Text User Interfaces (TUI) with Rust, from beginner up to
near‑expert level. This repository is designed to be clean and ready to be
published to GitHub as a portfolio project.
...
- `src/`
  - You are expected to add your own `main.rs` and binaries as you work through
    the exercises described in this document.

- `src/bin/`
  - Directory for separate binaries for each exercise.
  - Each `.rs` file here can be run with:
...
`exercise_name` is explained in each exercise section below.
...
- Avoid unnecessary comments; prefer clear variable and function names so the intent
  is readable from the code itself.
- Separate:
  - Application state (structs, enums).
  - Input‑handling functions.
  - Rendering functions.
- Use `match` to handle different keys in a clean way.
- Prioritize stability:
  - Handle potential `None` values or out‑of‑bounds indices carefully.


Development and Versioning Workflow
-----------------------------------

- Add one new `.rs` file in `src/bin` for each larger exercise.
- Use descriptive binary names, for example:
  - `hello_tui.rs`
  - `layout_demo.rs`
  - `list_menu.rs`
  - `todo_tui.rs`
- After completing each exercise:
  - Run `cargo check` to ensure there are no compilation errors.
  - Run `cargo run --bin <name>` to test it interactively.


Closing Notes
-------------

This document is intended as a comprehensive practice guide, from basic concepts
up to building small TUI applications that are close to production quality.
Each level builds on the previous one, so it is recommended to follow the
exercises in order.

If you use this repository as part of your GitHub portfolio, add a short
description at the top of the GitHub repository page explaining the purpose
of this project and list which exercise levels you have already completed,
for example as a checklist in the repository description or README.
